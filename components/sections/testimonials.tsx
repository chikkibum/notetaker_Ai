...
const TestimonialCard = ({
  ...
  return (
    <div className="relative w-full max-w-xs overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-white/5 to-white/[0.02] p-10 shadow-[0px_2px_0px_0px_rgba(255,255,255,0.1)_inset]">
      ...
      <div className="text-white/90 text-base leading-relaxed">{body}</div>

      <div className="mt-5 flex items-center gap-2">
        <img src={img || "/placeholder.svg"} alt={name} height="40" width="40" className="h-10 w-10 rounded-full" />
        <div className="flex flex-col">
          <div className="leading-5 font-medium tracking-tight text-sm text-white">{name}</div>
          <div className="leading-5 tracking-tight text-sm text-white/60">{username}</div>
        </div>
      </div>
    </div>
  )
...
export function TestimonialsSection() {
  const testimonialsConfig = siteConfig.testimonials
        ...
        <div className="mx-auto max-w-[540px]">
          <div className="flex justify-center">
            ...
            >
              ...
              <span className="relative text-white">{testimonialsConfig.badge.text}</span>
            </button>
          </div>
